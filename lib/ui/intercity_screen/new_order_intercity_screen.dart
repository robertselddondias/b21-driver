  ...
  offerAcceptDialog(BuildContext context, IntercityController controller, InterCityOrderModel orderModel) {
    return showModalBottomSheet(
        ...
        builder: (context) {
          return Container(
            decoration:
                BoxDecoration(color: Theme.of(context).colorScheme.surface, borderRadius: const BorderRadius.only(topRight: Radius.circular(15), topLeft: Radius.circular(15))),
                          ...
                          onPress: () async {
                            if (double.parse(controller.driverModel.value.walletAmount.toString()) >= double.parse(Constant.minimumAmountToWithdrawal)) {
                              ...
                              ShowToastDialog.showToast(
                                  "Você precisa de um valor mínimo de ${Constant.amountShow(amount: Constant.minimumDepositToRideAccept)} na carteira para aceitar o pedido e fazer um lance".tr);
                            }
                          },
                        ),
                        const SizedBox(
